    ...
    function getUrlParameter(name) {
        ...
        return paramValue;
            ...
            if(response && response.status === "success") {
                ...
                // Purchase Event - Ensuring Magento Dynamic Order Data
                if(response.eventType === "purchase" && response.order_id) {
                    fbq('track', 'Purchase', {
                        value: response.total_price, // Magento order total
                        currency: response.currency || 'AUD',
                        content_ids: response.product_ids, // Magento product IDs in the order
                        content_type: 'product'
                    });
                }

                // Add to Cart Event
                if(response.eventType === "add_to_cart" && response.product_id) {
                    fbq('track', 'AddToCart', {
                        content_ids: [response.product_id],
                        content_type: 'product',
                        value: response.price, // Magento product price
                        currency: response.currency || 'AUD'
                    });
                }

                // Initiate Checkout (Abandoned Cart) - Triggered when checkout starts
                if(response.eventType === "checkout" && response.cart_value) {
                    fbq('track', 'InitiateCheckout', {
                        value: response.cart_value, // Magento cart total
                        currency: response.currency || 'AUD',
                        content_ids: response.cart_product_ids, // Magento cart product IDs
                        content_type: 'product'
                    });
                }
            }
            ...
            if(response && response.status === "success") {
                ...
                if(response.eventType === "purchase") {
                    fbq('track', 'Purchase', {
                        value: response.amount, // Ensure correct dynamic value
                        currency: 'AUD'
                    });
                }
                // Example: Tracking Add to Cart
                if(response.eventType === "add_to_cart") {
                    fbq('track', 'AddToCart', {
                        value: response.amount,
                        currency: 'AUD'
                    });
                }
                // Example: Tracking Page View (as a general fallback)
                fbq('track', 'PageView');
            }
                                success: function(response) {
